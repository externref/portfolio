...
<section class="mx-auto my-8 w-full max-w-full px-2 md:w-[40vw] md:max-w-2xl">
	<h2 class="text-primary mb-2 text-center text-xl font-bold sm:text-2xl">Live Activity</h2>
	<div class="bg-base-100 dark:bg-base-200 flex flex-col items-center rounded-lg p-4 shadow">
		<div class="mb-2 flex w-full items-center gap-3">
			<img
				src={data.avatar_url}
				alt={data.username}
				class="border-primary h-10 w-10 rounded-full border"
			/>
			<div class="flex-1 text-left">
				<div class="text-primary text-base leading-tight font-bold">
					&nbsp;&nbsp;{data.username}
				</div>
		...
		{#if data.activity.listening}
			<div class="mb-1 flex w-full flex-row items-center gap-3">
				<img
					...
					class="mr-2 h-10 w-10 shrink-0 rounded shadow"
				/>
				<div class="min-w-0 flex-1 text-left">
					<a
						...
						target="_blank"
						rel="noopener"
						class="text-primary block max-w-full truncate text-sm font-semibold hover:underline"
					>
						{data.activity.listening.title}
					</a>
					<div class="text-base-content/70 max-w-full truncate text-xs">
		...
		{#if data.activity.playing}
			<div class="mb-1 flex w-full flex-row items-center gap-3">
				...
				{/if}
				<div class="min-w-0 flex-1 text-left">
					<div class="text-primary max-w-full truncate text-sm font-semibold">
					...
					{#if data.activity.playing.url}
						<a
							...
							target="_blank"
							rel="noopener"
							class="text-xs text-blue-500 hover:underline">More Info</a
						>
					{/if}
				</div>
			</div>
		...
		{#if !data.activity.listening && !data.activity.playing && !data.activity.custom}
			<div class="text-base-content/60 text-center text-xs">No current activity</div>
		{/if}
	</div>
</section>
